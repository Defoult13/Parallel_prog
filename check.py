...
def plot_multiple_timings(files_labels_colors):
    ...
    for filename, label, color, linestyle in files_labels_colors:
        ...
        plt.plot(numeric_sizes, times, marker='o', label=label, color=color, linestyle=linestyle)

    plt.title("Сравнение времени перемножения матриц (CPU vs CUDA)")
...
def main():
    ...
    files_labels_colors = [
        ("timing_results.txt",       "CPU (1 поток)",        "blue",  "-"),
        ("timing_results(4th).txt",  "CPU (4 потока)",       "green", "--"),
        ("timing_results(8th).txt",  "CPU (8 потоков)",      "orange", "-."),
        ("timing_results(8).txt",    "CUDA (BlockSize = 8)", "red",   "-"),
        ("timing_results(16).txt",   "CUDA (BlockSize = 16)","purple","--")
    ]
